 ...
 * Paramètres (en sortie):
 *   - service : service demandé par l'application (T_CONNECT, T_DATA ou
 * T_DISCONNECT)
 *   - donnees : message devant être envoyé (issu de la couche application)
 *   - taille_msg : nombre d'octets de données à émettre (0 si pas de données)
 */
void de_application_mode_c(int *service, unsigned char *donnees,
                           int *taille_msg);
 ...
 * Paramètres (en entrée):
 *   - service : service remonté à la couche application (T_CONNECT, T_DATA ou
 * T_DISCONNECT)
 *   - donnees : données à remonter à l'application
 *   - taille_msg : taille des données (0 si pas de données)
 * Renvoie :
 *   - Si service = T_CONNECT
 *     --> La réponse à la demande de connexion (T_CONNECT_ACCEPT ou
 * T_CONNECT_REFUSE)
 *   - Si service ≠ T_CONNECT
 *     --> 0 si OK
 *   - Dans tous les cas : -1 si erreur
 */
int vers_application_mode_c(int service, unsigned char *donnees,
                            int taille_msg);
